...
import * as mermaidParser from '../src/mermaid-parser.js'

describe('markdown-it-textual-uml: mermaid', function () {
  it('test sanity', function () {
    md().use(umlPlugin)
    assert(
      md().render('# Hello world').trim() === '<h1>Hello world</h1>',
      '# Hello world',
    )
    assert(
      md().render('Hello world').trim() === '<p>Hello world</p>',
      'Hello world',
    )
  })
  ...
  it('test mermaid direct', async function () {
    ...
    const svg = await mermaidParser.default.awaitRender(`mermaid
    graph TD
  ...
  it('test logging', function () {
    this.timeout(5000)
    console.log(
      md().use(umlPlugin).render(`\`\`\`mermaid
    graph TD
        A[Christmas] -->|Get money| B(Go shopping)
        B --> C{Let me think}
        C -->|One| D[Laptop]
